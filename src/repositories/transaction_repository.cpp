...
{  
    public:
        ...
        TransactionListPtr getByCustomerId(std::int32_t customerId, int limit) const
        {
            auto result = std::make_shared<std::vector<Transaction>>();
            try
            {
                auto session = this->_sessionPool->get();
                Poco::Data::Statement statement(session);
            ...
            {
                std::cerr << "[TransactionRepository::getByCustomerId]" << e.displayText() << '\n';
            }   
